...
export function registerLabelTools(server: McpServer, client: VikunjaClientApi): void {
  ...
  server.registerTool(
    ...
    withToolLogging("label_get", async ({ label_id }) => {
      try {
  ...
  server.registerTool(
    ...
    withToolLogging("label_create", async (args) => {
      try {
  ...
  server.registerTool(
    ...
    withToolLogging("label_update", async (args) => {
      try {
        const { label_id } = args;
        const patch = buildLabelPatch(args);
  ...
  server.registerTool(
    ...
    withToolLogging("label_delete", async ({ label_id, confirm }) => {
      try {
  ...
  server.registerTool(
    ...
    withToolLogging("task_labels_list", async ({ task_id, page, per_page, search }) => {
      try {
  ...
  server.registerTool(
    ...
    withToolLogging("task_remove_label", async ({ task_id, label_id }) => {
      try {
        const currentLabels = await client.listTaskLabels(task_id, {
          perPage: verificationPageSize,
        });
